...
const SearchMetier = ({setIsMetierActive, setMetierSecteur, setFilterMetierDisplay} : Props) => {
    ...
    const renderMetierLink = (metier: Metier) => (
      <p key={metier.id} onClick={(event: React.MouseEvent<HTMLParagraphElement, MouseEvent>) => {metierSelection(event)}} className='group nav-links-metier-item flex items-center justify-between font-normal hover:font-semibold'>
        {metier.nameMetier}
